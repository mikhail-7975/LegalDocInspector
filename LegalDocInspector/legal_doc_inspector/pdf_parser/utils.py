...
def get_pdf_files(directory):
    ...
    return [
        ...
        if file.is_file() and file.suffix.lower() == '.pdf'  # Проверка типа файла и расширения
    ]
    ...
    conversation =[
            {
                "role": "system",
                "content": [
                    {"type": "text", "text": f"Ты - ассистент для обработка документов. Вот содержание документа: {pdf_text} Вот примеры названия документов: {rev_chunks}. Ответь на запрос пользователя"}
                ],
            },
            {
                "role": "user",
                "content": [{"type": "text", "text": f"Напиши название документа"}],
            }
        ]
    return conversation
