...
def calculate_penalty(debt, start_date, end_date, consumer_type, payment_amount=0, payment_date=None):
    ...
    if consumer_type == "Управляющая компания":
        ...
        if delta > 60:
            ...
            print(f"{period1_end.strftime('%Y-%m-%d')} -> {period2_end.strftime('%Y-%m-%d')}: {days2} дней, Пени: {penalty2:.2f} руб., Долг: {debt:.2f} руб.")
        ...
        if delta > 90:
            ...
            print(f"{period2_end.strftime('%Y-%m-%d')} -> {end_date.strftime('%Y-%m-%d')}: {days3} дней, Пени: {penalty3:.2f} руб., Долг: {debt:.2f} руб.")
        ...
        penalty1 = debt * days1 * rate1 * CB_RATE
        total_penalty += penalty1
        print(f"{start_date.strftime('%Y-%m-%d')} -> {end_date.strftime('%Y-%m-%d')}: {days1} дней, Пени: {penalty1:.2f} руб., Долг: {debt:.2f} руб.")
