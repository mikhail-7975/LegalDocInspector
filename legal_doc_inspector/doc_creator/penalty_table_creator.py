...
class PenaltyTableCreator:
    ...
    def create_penalty_table_from_json(
        self, name, data, contract_number, start_date, end_date
    ):
        table = self._create_table_title(contract_number, start_date, end_date)
        list_of_periods = self.group_by_month(data)
        table, contract_number, all_debt, all_penalty = self._create_penalty_table(
            table, list_of_periods, contract_number
        )
    ...
    def create_result_table(self, list_of_tables_info: list[tuple], name: Path):
        ...
        self._put_text_into_table_cell(
            text="Период", cell=title_row_cells[1], font_size=12, need_bold=True
        )

        self._put_text_into_table_cell(
            text="Задолженность (основной долг), руб.",
        ...
        self._put_text_into_table_cell(
            text="Неустойка,руб.", cell=title_row_cells[3], font_size=12, need_bold=True
        )
        ...
        for contract_number, start_date, end_date, debt, penalty in list_of_tables_info:
            self.contracts_info[contract_number]["penalty_period"] = (
            ...
            self._put_text_into_table_cell(
                text=f"{self.contracts_info[contract_number]['contract_periods']}",
                font_size=11,
                cell=row_cells[1],
            )

            self._put_text_into_table_cell(
                text=f"{self.format_float_to_currency(debt)}",
                cell=row_cells[2],
                font_size=11,
            )

            self._put_text_into_table_cell(
                text=f"{self.format_float_to_currency(penalty)}",
                cell=row_cells[3],
                font_size=11,
            )

            self._put_text_into_table_cell(
                text=f"{self.format_float_to_currency(float(debt) + float(penalty))}",
                cell=row_cells[4],
                font_size=11,
            )

            self.contracts_info[contract_number]["debt"] = (
                self.format_float_to_currency(debt)
            )
            self.contracts_info[contract_number]["penalty"] = (
                self.format_float_to_currency(penalty)
            )
            self.contracts_info[contract_number]["debt_penalty"] = (
                self.format_float_to_currency(float(debt) + float(penalty))
            )
        ...
        self._put_text_into_table_cell(
            text=f"{self.format_float_to_currency(all_debt)}",
            font_size=11,
            need_bold=True,
        ...
        self._put_text_into_table_cell(
            text=f"{self.format_float_to_currency(all_penalty)}",
            font_size=11,
            need_bold=True,
        ...
        self._put_text_into_table_cell(
            text=f"{self.format_float_to_currency(float(all_debt) + float(all_penalty))}",
            font_size=11,
            need_bold=True,
        ...
        self.contracts_info["cost_of_lawsuit"] = self.format_float_to_currency(
            float(all_debt) + float(all_penalty)
        )
    ...
    def _create_table_title(self, contract_number: str, start_date, end_date):
        paragraph = self.doc.add_paragraph()
        run = paragraph.add_run(
            f"Исходя из размера Основного долга и Периода задолженности, неустойка по состоянию на {end_date} составляет:"
        )
        run.font.name = "Times New Roman"
        element = run._element
        rPr = element.get_or_add_rPr()
        ...
        self._put_text_into_table_cell(
            "Информация о расчёте", row_cells[0], need_gray_bgc=True, orient="left"
        )

        row_cells[3].merge(row_cells[10])

        self._put_text_into_table_cell(
            text=f"{contract_number}", cell=row_cells[3], orient="left", need_bold=True
        )
        ...
        self._put_text_into_table_cell(
            text=f"{start_date}", cell=row_cells[2], orient="left"
        )

        row_cells[5].merge(row_cells[6])

        self._put_text_into_table_cell(
            text="конец просрочки", cell=row_cells[5], orient="left", need_gray_bgc=True
        )

        row_cells[7].merge(row_cells[10])

        self._put_text_into_table_cell(
            text=f"{end_date}", cell=row_cells[7], orient="left"
        )
        ...
        self._put_text_into_table_cell(
            text="Месяц", need_bold=True, cell=up_row_cells[0]
        )
        ...
        self._put_text_into_table_cell(
            text="Долг", need_bold=True, cell=up_row_cells[2]
        )

        up_row_cells[4].merge(up_row_cells[6])

        self._put_text_into_table_cell(
            text="Период просрочки", cell=up_row_cells[4], need_bold=True
        )

        self._put_text_into_table_cell(text="с", need_bold=True, cell=down_row_cells[4])

        self._put_text_into_table_cell(
            text="по", need_bold=True, cell=down_row_cells[5]
        )

        self._put_text_into_table_cell(
            text="дней", need_bold=True, cell=down_row_cells[6]
        )

        up_row_cells[7].merge(down_row_cells[7])

        self._put_text_into_table_cell(
            text="Ставка", need_bold=True, cell=up_row_cells[7]
        )

        up_row_cells[8].merge(down_row_cells[8])

        self._put_text_into_table_cell(
            text="Доля ставки", need_bold=True, cell=up_row_cells[8]
        )

        up_row_cells[9].merge(down_row_cells[9])

        self._put_text_into_table_cell(
            text="Формула", need_bold=True, cell=up_row_cells[9]
        )

        up_row_cells[10].merge(down_row_cells[10])

        self._put_text_into_table_cell(
            text="Пени", need_bold=True, cell=up_row_cells[10]
        )
    ...
    def group_by_month(self, data):
        ...
        for entry in data:
            ...
            period_data = {
                k: v for k, v in entry.items() if k not in ("payments", "month")
            }
    ...
    def _create_penalty_table(self, table: Table, list_dict_of_months, contract_number):
        ...
        for month_dict in list_dict_of_months:
            ...
            for period_or_payment in periods_and_payments:
                if period_or_payment["type"] == "period":
                    ...
                    self._put_text_into_table_cell(
                        text=f"{start_date}", cell=new_row_cells[4]
                    )

                    self._put_text_into_table_cell(
                        text=f"{end_date}", cell=new_row_cells[5]
                    )

                    self._put_text_into_table_cell(
                        text=f"{days_count}", cell=new_row_cells[6]
                    )

                    self._put_text_into_table_cell(text="9,50%", cell=new_row_cells[7])

                    self._put_text_into_table_cell(text=rate, cell=new_row_cells[8])

                    self._put_text_into_table_cell(
                        text=f"{self.format_float_to_currency(debt)} × {days_count} × {rate} × 9,5%",
                        cell=new_row_cells[9],
                    )

                    self._put_text_into_table_cell(
                        text=f"{self.format_float_to_currency(penalty)} р.",
                ...
                if period_or_payment["type"] == "payment":
                    ...
                    new_row_cells[2].merge(new_row_cells[3])
                    self._put_text_into_table_cell(
                        text=f"-{self.format_float_to_currency(amount)}",
                        cell=new_row_cells[2],
                    )

                    self._put_text_into_table_cell(
                        text=f"{date_of_payment}", cell=new_row_cells[4]
                    )
            ...
            self._put_text_into_table_cell(
                text=f"{self.format_float_to_currency(itogo)} р.",
        ...
        self.contracts_info[contract_number]["contract_periods"] = (
            f"{month_year_to_decimal(months[0])}-{month_year_to_decimal(months[-1])}"
        )
        ...
        self._put_text_into_table_cell(
            text=f"Сумма основного долга: {self.format_float_to_currency(all_debt)} руб.",
        ...
        self._put_text_into_table_cell(
            text=f"Сумма пеней по всем задолженностям: {self.format_float_to_currency(all_penalty)} руб.",
