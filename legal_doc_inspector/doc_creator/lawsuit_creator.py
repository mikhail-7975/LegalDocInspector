...
class LawsuitCreator:
    ...
    def _get_service_type5(self):
        res = ''
        for uid, contract_name, content in self.contracts:
            if "ТЭ" in contract_name:
                res = res + "ТЭ"
            if "ГВС" in contract_name:
                res = res + "ГВС"
        return res 
    ...
    def create_third_part_of_lawsuit(self, info_json):
        ...
        # брать тип компании из info=info_json['company_type']

        service_type = self._get_service_type5()
        company_type = info_json['company_type']
        if company_type == "ТСЖ":
