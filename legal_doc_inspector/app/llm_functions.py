...
def parse_contract(file, parser):
    ...
    service_type = parser.find_info(
        contract_text_chunks,
        "Какой тип услуги указан в договоре? Выбери: вода, отопление или оба, ответь одним словом?",
        service_flag=True,
    )
    # определяем фрагмент с датой начала просрочки
    overdue_date = parser.find_info(
        contract_text_chunks,
        "В какой срок исполнитель должен произвести оплату?",
        service_flag=False,
    )
    # опредляем номер договора
    contract_number = parser.find_info(
        contract_text_chunks, "номер договора №", service_flag=True
    )
    contract_number = contract_number.split(":", 1)[-1].strip()
    # определяем тип услуги по ответу сети
    if service_type == "Оба." or service_type == "Оба":
        service_type = "тепловую энергию/теплоноситель (ТЭ) и горячую воду (ГВС))"
    if service_type == "Вода." or service_type == "Вода":
        service_type = "горячую воду (ГВС))"
    if service_type == "Отопление." or service_type == "Отопление":
        service_type = "тепловую энергию/теплоноситель (ТЭ)"
...
def parse_claim(file, parser):
    ...
    lines = res.split("\n")
    parsed_lines = [
        line.split(":", 1)[1].strip() if ":" in line else line for line in lines
    ]
    # получаем инн истца
