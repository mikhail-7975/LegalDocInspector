...
import pandas as pd
from utils.HolidayChecker import (
    HolidayChecker,  # Импортируем наш класс для проверки праздников
)
...
def calculate_penalty(
    ...
    if consumer_type == "Управляющая компания":
        ...
        period1_start = get_next_workday_after_n_days(
            start_date, 1
        )  # Первый рабочий день
        period1_end = get_next_workday_after_n_days(
            period1_start, 59
        )  # +59 рабочих дней = всего 60
        ...
        if end_date > period1_end:
            ...
            period2_end = get_next_workday_after_n_days(
                period2_start, 29
            )  # +29 рабочих дней = всего 30 (61-90)
            ...
            if days2 > 0:
                ...
                print(
                    f"{period2_start.strftime('%Y-%m-%d')} -> {period2_end.strftime('%Y-%m-%d')}: {days2} дней, Пени: {penalty2:.2f} руб., Долг: {debt:.2f} руб."
                )
        ...
        if end_date > period2_end:
            ...
            if days3 > 0:
                ...
                print(
                    f"{period3_start.strftime('%Y-%m-%d')} -> {end_date.strftime('%Y-%m-%d')}: {days3} дней, Пени: {penalty3:.2f} руб., Долг: {debt:.2f} руб."
                )
        ...
        if end_date > period1_end:
            ...
            period2_end = get_next_workday_after_n_days(
                period2_start, 59
            )  # +59 рабочих дней = всего 60 (31-90)
        ...
        if days1 > 0:
            ...
            total_penalty += penalty1
            print(
                f"{period1_start.strftime('%Y-%m-%d')} -> {end_date.strftime('%Y-%m-%d')}: {days1} дней, Пени: {penalty1:.2f} руб., Долг: {debt:.2f} руб."
            )
