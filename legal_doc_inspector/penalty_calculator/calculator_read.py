...
# округлять в +; ввести праздники; учет отрицательных уплат?; интегрировать с календарем; парсить доки на начало и конец периода; сд фронтэнд
def calculate_penalty(
    debt, start_date, end_date, consumer_type, payment_amount=0, payment_date=None
):
    """
    ...
    if payment_date:
        if payment_date < start_date:
            ...
            debt -= payment_amount
            print(
                f"Оплата {payment_amount} руб. произведена до начала периода {start_date.strftime('%Y-%m-%d')}. Новый долг: {debt:.2f} руб."
            )
        elif start_date <= payment_date <= end_date:
            # Если оплата была произведена в период просрочки, уменьшаем долг на момент оплаты
            debt -= payment_amount
            print(
                f"Оплата {payment_amount} руб. произведена {payment_date.strftime('%Y-%m-%d')}. Новый долг: {debt:.2f} руб."
            )
        else:
            # Если оплата была произведена после окончания периода, она не учитывается
            print(
                f"Оплата {payment_amount} руб. произведена после окончания периода {end_date.strftime('%Y-%m-%d')} и не учитывается."
            )

    # Определяем подпериоды в зависимости от типа потребителя
    if consumer_type == "Управляющая компания":
        # Подпериод 1: с 1 по 60 день
        period1_end = start_date + timedelta(days=60)
        days1 = min((period1_end - start_date).days, delta)
        rate1 = 1 / 300
        penalty1 = debt * days1 * rate1 * CB_RATE
        total_penalty += penalty1
        print(
            f"{start_date.strftime('%Y-%m-%d')} -> {period1_end.strftime('%Y-%m-%d')}: {days1} дней, Пени: {penalty1:.2f} руб., Долг: {debt:.2f} руб."
        )

        # Подпериод 2: с 61 по 90 день
        if delta > 60:
            period2_end = start_date + timedelta(days=90)
            days2 = min((period2_end - period1_end).days, delta - 60)
            rate2 = 1 / 170
            penalty2 = debt * days2 * rate2 * CB_RATE
            total_penalty += penalty2
            print(
                f"{period1_end.strftime('%Y-%m-%d')} -> {period2_end.strftime('%Y-%m-%d')}: {days2} дней, Пени: {penalty2:.2f} руб., Долг: {debt:.2f} руб."
            )

        # Подпериод 3: с 91 дня
        if delta > 90:
            days3 = delta - 90
            rate3 = 1 / 130
            ...
            print(
                f"{period2_end.strftime('%Y-%m-%d')} -> {end_date.strftime('%Y-%m-%d')}: {days3} дней, Пени: {penalty3:.2f} руб., Долг: {debt:.2f} руб."
            )

    elif consumer_type in ["ТСЖ", "ЖСК", "ТСН"]:
        # Подпериод 1: с 1 по 30 день
        period1_end = start_date + timedelta(days=30)
        days1 = min((period1_end - start_date).days, delta)
        ...
        # Подпериод 2: с 31 по 90 день
        if delta > 30:
            period2_end = start_date + timedelta(days=90)
            days2 = min((period2_end - period1_end).days, delta - 30)
            rate2 = 1 / 300
            penalty2 = debt * days2 * rate2 * CB_RATE
            total_penalty += penalty2
            print(
        ...
        total_penalty += penalty1
        print(
            f"{start_date.strftime('%Y-%m-%d')} -> {end_date.strftime('%Y-%m-%d')}: {days1} дней, Пени: {penalty1:.2f} руб., Долг: {debt:.2f} руб."
        )
...
if any(
    keyword in consumer_te_lower
    for keyword in ["управляющая компания", "управляющая организация"]
):
    consumer_type = "Управляющая компания"
elif any(
    keyword in consumer_te_lower
    for keyword in ["товарищество собственников жилья", "тсж"]
):
    consumer_type = "ТСЖ"
elif any(
    keyword in consumer_te_lower
    ...
    keyword in consumer_te_lower
    for keyword in ["товарищество собственников недвижимости", "тсн"]
):
    consumer_type = "ТСН"
...
for i in range(len(df)):
    ...
    if pd.notna(month) and isinstance(month, str) and "." in month:
        ...
        )  # Если задолженность не указана, считаем её равной 0
        ...
        if i + 1 < len(df):  # Проверяем, что следующая строка существует
            ...
            if pd.notna(
                next_row[2]
            ):  # Если в следующей строке есть дата оплаты (столбец "C")
                payment_date = next_row[2]  # Дата оплаты
                payment = (
                    next_row[3] if pd.notna(next_row[3]) else 0
                )  # Сумма оплаты (столбец "D")
...
first_date = datetime(
    year=int(first_year), month=int(first_month), day=1
)  # Первый день месяца
...
for month_year, debt, payment, payment_date in zip(
    months, debts, payments, payment_dates, strict=False
):
    month, year = month_year.split(".")
    # Дата начала просрочки: 18 число следующего месяца
    start_date = (
        datetime(int(year), int(month) + 1, 18)
        if int(month) < 12
        else datetime(int(year) + 1, 1, 18)
    )
    ...
    penalty, delta = calculate_penalty(
        debt,
    ...
    print(
        f"Месяц: {month_year}, Задолженность: {debt}, Оплата: {payment}, Дата оплаты: {payment_date}, Дней от 18 числа следующего месяца до 25.11.2024: {days_from_18}, Пени: {penalty:.2f} руб."
    )
