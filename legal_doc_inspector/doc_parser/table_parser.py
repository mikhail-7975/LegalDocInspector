...
class TableParser:
    ...
    def add_entry(
        self, data, month_year, invoice_amount, oplata, text_month, payments=None
    ):
        """
    ...
    def _find_inn(self, df: pd.DataFrame):
        matches = df[
            df.iloc[:, 3].str.contains(self.inn_pattern, regex=True, na=False)
        ].index.to_list()[0]
    ...
    def _parse_contract_number(self, df: pd.DataFrame):
        matches = df[
            df.iloc[:, 0].str.contains(self.number_pattern, regex=True, na=False)
        ].index.to_list()[0]

        contract_number = df.iloc[matches, 1]

        matches = df[
            df.iloc[:, 0].str.contains(r"Дата :", regex=True, na=False)
        ].index.to_list()[0]
    ...
    def parse_excel_table(self, path_to_table: Path):
        ...
        matches = df[
            df.iloc[:, 0].str.contains(self.month_year_pattern, regex=True, na=False)
        ]
        ...
        for i, start_index in enumerate(row_indexes_to_search):
            ...
            for row_index in range(start_index + 1, end_index):
                ...
                text_month_match_id = (
                    df.iloc[:start_index, 0]
                    .str.contains(self.text_month_pattern, regex=True, na=False)
                    .index.to_list()[-1]
                )
                ...
                if re.match(self.currency_pattern, str(vystavleny_schet)) and re.match(
                    self.currency_pattern, str(oplata)
                ):
                    ...
                    if int(oplata) > 0:
                        ...
                        for row_index_2 in range(start_index + 1, row_index):
                            if re.match(
                                self.date_pattern, str(df.iloc[row_index_2, 2])
                            ) and re.match(
                                self.currency_pattern, str(df.iloc[row_index_2, 3])
                            ):
                                payments_list.append(
                                    (
                                        str(df.iloc[row_index_2, 2]),
                                        df.iloc[row_index_2, 3],
                                    )
                                )
